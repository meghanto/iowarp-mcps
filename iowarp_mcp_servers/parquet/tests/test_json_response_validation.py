...
async def test_json_data_matches_inequality_filter():
    ...
    assert data["status"] == "success"
    for row in data["data"]:
        assert row["batch_id"] != 1, f"Row has batch_id={row['batch_id']}, should not be 1"
...
async def test_json_data_matches_greater_than_filter():
    ...
    assert data["status"] == "success"
    for row in data["data"]:
        assert row["azimuth"] > 5.0, f"Row has azimuth={row['azimuth']}, should be > 5.0"
...
async def test_empty_filter_returns_valid_json():
    ...
    filter_spec = json.dumps({"column": "zenith", "op": "less", "value": 0})  # Impossible
    result = await read_slice(
        file_path="datasets/train_meta.parquet",
        start_row=0,
        end_row=200,
