...
async def test_filtered_slice_empty_result():
    ...
    filter_spec = json.dumps({"column": "zenith", "op": "less", "value": 0})  # Impossible (zenith >= 0)
...
async def test_filtered_slice_all_match():
    ...
    filter_spec = json.dumps({"column": "event_id", "op": "greater", "value": 0})  # All should match
    result = await read_slice(
        file_path="datasets/train_meta.parquet",
        start_row=0,
        end_row=100,
...
async def test_filtered_slice_size_limit():
    ...
    filter_spec = json.dumps({"column": "charge", "op": "greater", "value": 0.1})  # Matches most rows
    result = await read_slice(
        file_path="datasets/batch_1.parquet",
        start_row=0,
    ...
    if data["status"] == "error":
        assert "payload" in data["message"].lower() or "limit" in data["message"].lower()
...
async def test_filtered_slice_column_not_found():
    ...
    filter_spec = json.dumps({"column": "nonexistent_column", "op": "greater", "value": 5})
    result = await read_slice(
        file_path="datasets/batch_1.parquet",
        start_row=0,
        end_row=100,
...
async def test_filtered_slice_type_mismatch():
    ...
    filter_spec = json.dumps({"column": "sensor_id", "op": "equal", "value": "abc"})  # String for int column
    result = await read_slice(
        file_path="datasets/batch_1.parquet",
        start_row=0,
        end_row=100,
