...
uv --directory=$CLONE_DIR/iowarp-mcps/iowarp_mcp_servers/${name.toLowerCase().replace(/ /g, '-')} run ${name.toLowerCase().replace(/ /g, '-')}-mcp --help
...
$env:CLONE_DIR=$PWD
git clone https://github.com/iowarp/iowarp-mcps.git
uv --directory=$env:CLONE_DIR\\iowarp-mcps\\iowarp_mcp_servers\\${name.toLowerCase().replace(/ /g, '-')} run ${name.toLowerCase().replace(/ /g, '-')}-mcp --help`
    }
  };
  ...
  return (
    <div className={styles.mcpDetail}>
      ...
      <div className={styles.header}>
        <Link to="/iowarp-mcps" className={styles.backButton}>
          ← Back to MCPs
        </Link>
