...
class TestRecordHandler:
    ...
    async def test_record_interaction_basic(self):
        ...
        chronicle_name = f"test_chronicle_{int(time.time())}_{random.randint(1000, 9999)}"
        ...
        assert "ChronoLog session started" in start_result, f"Expected session to start, got: {start_result}"
        
        # Record interaction
        result = await record_interaction("test user message", "test assistant response")
        assert isinstance(result, str)
        assert "Interaction recorded to ChronoLog" in result, f"Expected interaction to be recorded, got: {result}"
        
        # Stop session
        stop_result = await stop_chronolog()
        assert isinstance(stop_result, str)
        assert "ChronoLog session stopped" in stop_result, f"Expected session to stop, got: {stop_result}"
