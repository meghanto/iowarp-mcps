...
from .utils import MockPipeline, MockJarvisManager, temporary_directory, TestDataGenerator
...
class TestJarvisIntegration:
    ...
    async def test_complete_pipeline_workflow(self):
        ...
        with patch('jarvis_cd.basic.jarvis_manager.JarvisManager') as mock_jarvis, \
            ...
            async def append_pkg_tool(pipeline_id: str, package_type: str, pkg_id: str = None):
                return {"pipeline_id": pipeline_id, "package_type": package_type, "pkg_id": pkg_id, "status": "appended"}
            
            async def configure_pkg_tool(pipeline_id: str, pkg_id: str, **kwargs):
                return {"pipeline_id": pipeline_id, "pkg_id": pkg_id, "config": kwargs, "status": "configured"}
    ...
    async def test_jarvis_manager_workflow(self):
        ...
        with patch('jarvis_cd.basic.jarvis_manager.JarvisManager') as mock_jarvis:
            ...
            # Mock manager tools
            async def jm_create_config(config_dir: str, private_dir: str, shared_dir: str = None):
            ...
            init_result = await jm_create_config(self.config_dir, self.private_dir, self.shared_dir)
    ...
    async def test_error_handling_workflow(self):
        ...
        with patch('jarvis_cd.basic.jarvis_manager.JarvisManager') as mock_jarvis, \
            ...
            async def failing_append_pkg(pipeline_id: str, package_type: str, pkg_id: str = None):
    ...
    async def test_concurrent_operations(self):
        ...
        with patch('jarvis_cd.basic.jarvis_manager.JarvisManager') as mock_jarvis, \
            ...
            tasks = [
                create_pipeline_tool(f"pipeline_{i}") for i in range(5)
            ] + [
    ...
    async def test_configuration_persistence(self):
        ...
        with temporary_directory() as temp_dir:
            ...
            with patch('jarvis_cd.basic.jarvis_manager.JarvisManager') as mock_jarvis:
                ...
                create_result = await jm_create_config(config_dir, private_dir, shared_dir)
                assert create_result["status"] == "created"
