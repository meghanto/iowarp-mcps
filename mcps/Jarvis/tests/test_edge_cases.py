...
from tests.utils import (
    MockPipeline, MockJarvisManager
)
...
class TestEdgeCases:
    ...
    def test_nested_configuration_structures(self):
        ...
        with patch('sys.path'):
            ...
            assert mock_pipeline.packages[0]["kwargs"]["level1"]["level2"]["level3"]["level4"] == "deep_value"
...
class TestErrorRecovery:
    ...
    def test_partial_package_failure_recovery(self):
        ...
        with patch('sys.path'):
            ...
            current_packages = [p for p in mock_pipeline.packages if p.get("type") != "processor1"]
            # Replace the packages with filtered list
            mock_pipeline._packages_list = current_packages
            # Update dict representation too
            mock_pipeline._packages_dict = {p.get("id", p.get("type", "unknown")): {"type": p.get("type"), "config": p.get("kwargs", {})} for p in current_packages}
    ...
    def test_concurrent_modification_conflicts(self):
        ...
        with patch('sys.path'):
            ...
            original_packages = list(mock_pipeline.packages)  # Use list() instead of copy()
