...
class MockJarvisManager:
    ...
    def __init__(self):
        ...
        self.pipelines = ["pipeline1", "pipeline2", "pipeline3"]  # Default pipelines for testing
    ...
    def create(self, config_dir: str, private_dir: str, shared_dir: Optional[str] = None):
        """Mock create method."""
        self.config = {
...
class TestDataGenerator:
    ...
    @staticmethod
    def generate_pipeline_config(pipeline_id: str = "test_pipeline", num_packages: int = 2, size: str = "small") -> Dict[str, Any]:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for operation in self.operations:
            summary[operation] = {
                ...
                "success_count": len([op for op in self.operations[operation] if op["success"]]),
...
class ServerToolTester:
    ...
    async def call_tool_function(tool, *args, **kwargs):
        ...
        if hasattr(tool, 'fn'):
            ...
            return await tool.fn(*args, **kwargs) if asyncio.iscoroutinefunction(tool.fn) else tool.fn(*args, **kwargs)
        elif callable(tool):
...
def assert_valid_pipeline_response(response: Dict[str, Any], expected_pipeline_id: str = None, expected_status: str = None):
...
def assert_valid_manager_response(response: Union[Dict[str, Any], List[Dict[str, Any]]]):
