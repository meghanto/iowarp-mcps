...
class TestArxivMCPServer:
    ...
    def test_main_function_runs_sse(self, mock_mcp):
        ...
        with patch.dict(os.environ, {"MCP_TRANSPORT": "sse", "MCP_SSE_HOST": "localhost", "MCP_SSE_PORT": "8080"}):
            server.main()
    ...
    @patch.dict(os.environ, {"MCP_TRANSPORT": "SSE", "MCP_SSE_HOST": "test.host", "MCP_SSE_PORT": "9999"})
    def test_transport_selection_sse(self):
        """Test that SSE transport is selected correctly."""
        with patch('server.mcp') as mock_mcp:
            mock_mcp.run = MagicMock()
            server.main()
            mock_mcp.run.assert_called_with(transport="sse", host="test.host", port=9999)
    ...
    @patch('server.logger')
    async def test_search_arxiv_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_get_recent_papers_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            mock_handler.assert_called_once_with("cs.AI", 5)
            # Verify logging was called
            mock_logger.info.assert_called_with("Getting recent papers from category: cs.AI")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.search_papers_by_author_handler')
    @patch('server.logger')
    async def test_search_papers_by_author_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_search_by_title_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            # Verify logging was called
            mock_logger.info.assert_called_with("Searching papers by title: neural networks")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.search_by_abstract_handler')
    @patch('server.logger')
    async def test_search_by_abstract_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            # Verify logging was called
            mock_logger.info.assert_called_with("Searching papers by abstract: machine learning")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.search_by_subject_handler')
    @patch('server.logger')
    async def test_search_by_subject_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            # Verify logging was called
            mock_logger.info.assert_called_with("Searching papers by subject: Computer Science")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.search_date_range_handler')
    @patch('server.logger')
    async def test_search_date_range_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            # Verify logging was called
            mock_logger.info.assert_called_with("Searching papers by date range: 2023-01-01 to 2023-12-31")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.get_paper_details_handler')
    @patch('server.logger')
    async def test_get_paper_details_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_find_similar_papers_tool_function_with_logging(self, mock_logger, mock_handler):
        ...
        if func and callable(func):
            ...
            # Verify logging was called
            mock_logger.info.assert_called_with("Finding similar papers for: 2023.12345")

    @pytest.mark.asyncio
    @patch('server.mcp_handlers.export_to_bibtex_handler')
    @patch('server.logger')
    async def test_export_to_bibtex_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_download_paper_pdf_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_get_pdf_url_tool_function_with_logging(self, mock_logger, mock_handler):
    ...
    @patch('server.logger')
    async def test_download_multiple_pdfs_tool_function_with_logging(self, mock_logger, mock_handler):
...
"""
    ...
    @patch('server.logger')
    async def test_direct_tool_function_calls_for_coverage(self, mock_logger, mock_handlers):
        ...
        mock_handlers.search_papers_by_author_handler = AsyncMock(return_value={"papers": []})
        mock_handlers.search_by_title_handler = AsyncMock(return_value={"papers": []})
        mock_handlers.search_by_abstract_handler = AsyncMock(return_value={"papers": []})
        ...
        mock_handlers.find_similar_papers_handler = AsyncMock(return_value={"similar_papers": []})
        mock_handlers.export_to_bibtex_handler = AsyncMock(return_value={"bibtex": ""})
        mock_handlers.download_paper_pdf_handler = AsyncMock(return_value={"status": "success"})
        mock_handlers.get_pdf_url_handler = AsyncMock(return_value={"pdf_url": ""})
        mock_handlers.download_multiple_pdfs_handler = AsyncMock(return_value={"status": "success"})
    ...
    async def test_execute_all_tool_functions_for_logger_coverage(self, mock_logger):
        ...
        with patch.object(server.mcp_handlers, 'search_arxiv_handler', new_callable=AsyncMock) as mock_search, \
             patch.object(server.mcp_handlers, 'get_recent_papers_handler', new_callable=AsyncMock) as mock_recent, \
             patch.object(server.mcp_handlers, 'search_papers_by_author_handler', new_callable=AsyncMock) as mock_author, \
             patch.object(server.mcp_handlers, 'search_by_title_handler', new_callable=AsyncMock) as mock_title, \
             patch.object(server.mcp_handlers, 'search_by_abstract_handler', new_callable=AsyncMock) as mock_abstract, \
             patch.object(server.mcp_handlers, 'search_by_subject_handler', new_callable=AsyncMock) as mock_subject, \
             patch.object(server.mcp_handlers, 'search_date_range_handler', new_callable=AsyncMock) as mock_date, \
             patch.object(server.mcp_handlers, 'get_paper_details_handler', new_callable=AsyncMock) as mock_details, \
             patch.object(server.mcp_handlers, 'find_similar_papers_handler', new_callable=AsyncMock) as mock_similar, \
             patch.object(server.mcp_handlers, 'export_to_bibtex_handler', new_callable=AsyncMock) as mock_bibtex, \
             patch.object(server.mcp_handlers, 'download_paper_pdf_handler', new_callable=AsyncMock) as mock_download, \
             patch.object(server.mcp_handlers, 'get_pdf_url_handler', new_callable=AsyncMock) as mock_url, \
             patch.object(server.mcp_handlers, 'download_multiple_pdfs_handler', new_callable=AsyncMock) as mock_multi:
    ...
    async def test_tool_function_parameters_validation(self):
        ...
        with patch.object(server, 'mcp_handlers') as mock_handlers:
            ...
            mock_handlers.search_arxiv_handler = AsyncMock(return_value={"papers": [], "count": 0})
            mock_handlers.get_recent_papers_handler = AsyncMock(return_value={"papers": []})
            mock_handlers.search_papers_by_author_handler = AsyncMock(return_value={"papers": []})
            mock_handlers.search_by_title_handler = AsyncMock(return_value={"papers": []})
            mock_handlers.search_by_abstract_handler = AsyncMock(return_value={"papers": []})
    ...
    async def test_error_handling_in_tools(self):
        ...
        with patch.object(server, 'mcp_handlers') as mock_handlers:
            ...
            mock_handlers.search_arxiv_handler = AsyncMock(side_effect=Exception("Search error"))
            mock_handlers.get_recent_papers_handler = AsyncMock(side_effect=ValueError("Invalid parameters"))
            mock_handlers.download_paper_pdf_handler = AsyncMock(side_effect=ConnectionError("Network error"))
    ...
    def test_environment_variable_parsing(self):
        ...
        with patch.dict(os.environ, {"MCP_SSE_PORT": "8080"}):
            with patch('server.mcp') as mock_mcp:
                ...
                with patch.dict(os.environ, {"MCP_TRANSPORT": "sse", "MCP_SSE_HOST": "localhost"}):
                    server.main()
