...
class TestFastMCPTools:
    ...
    async def test_server_tool_error_scenarios(self):
        ...
        with patch.object(server, 'mcp_handlers') as mock_handlers:
            ...
            mock_handlers.search_arxiv_handler = AsyncMock(side_effect=Exception("Handler error"))
            mock_handlers.get_recent_papers_handler = AsyncMock(side_effect=ValueError("Invalid parameters"))
            mock_handlers.search_papers_by_author_handler = AsyncMock(side_effect=ConnectionError("Network error"))
...
'''
    ...
    async def test_tool_function_comprehensive_coverage(self):
        ...
        with patch('server.logger'), \
            ...
            mock_handlers.search_arxiv_handler = AsyncMock(return_value={"papers": [{"id": "test"}], "count": 1})
            mock_handlers.get_recent_papers_handler = AsyncMock(return_value={"papers": [{"id": "recent"}]})
            mock_handlers.search_papers_by_author_handler = AsyncMock(return_value={"papers": [{"id": "author"}]})
            mock_handlers.search_by_title_handler = AsyncMock(return_value={"papers": [{"id": "title"}]})
            mock_handlers.search_by_abstract_handler = AsyncMock(return_value={"papers": [{"id": "abstract"}]})
            mock_handlers.search_by_subject_handler = AsyncMock(return_value={"papers": [{"id": "subject"}]})
            mock_handlers.search_date_range_handler = AsyncMock(return_value={"papers": [{"id": "date"}]})
            mock_handlers.get_paper_details_handler = AsyncMock(return_value={"paper": {"id": "details"}})
            mock_handlers.find_similar_papers_handler = AsyncMock(return_value={"similar_papers": [{"id": "similar"}]})
            mock_handlers.export_to_bibtex_handler = AsyncMock(return_value={"bibtex": "@article{test}"})
            mock_handlers.download_paper_pdf_handler = AsyncMock(return_value={"status": "success", "path": "/tmp/test.pdf"})
            mock_handlers.get_pdf_url_handler = AsyncMock(return_value={"pdf_url": "http://example.com/test.pdf"})
            mock_handlers.download_multiple_pdfs_handler = AsyncMock(return_value={"status": "success", "downloaded": 1})
            ...
            comprehensive_tests = [
                ...
                ('download_multiple_pdfs_tool', [['1234.5678', '2301.12345'], '/tmp/batch']),
            ]
