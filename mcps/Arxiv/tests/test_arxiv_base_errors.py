...
from capabilities.arxiv_base import execute_arxiv_query, generate_bibtex, parse_arxiv_entry
...
class TestArxivBaseErrors:
    ...
    async def test_parse_error(self, mock_client):
        ...
        with patch('capabilities.arxiv_base.ET.fromstring', side_effect=ET.ParseError("Invalid XML")):
            with pytest.raises(Exception) as exc_info:
                await execute_arxiv_query({"search_query": "test query", "max_results": 5})
    ...
    def test_parse_arxiv_entry_malformed_xml(self):
        ...
        assert "test.no_ns" in result["id"] or result["id"] == ""  # May not extract ID without namespace
        assert result["title"] == "No Namespace Entry" or result["title"] == ""  # May not parse without namespace        # Test with empty elements
        empty_elements_xml = """
        <entry xmlns="http://www.w3.org/2005/Atom">
    ...
    async def test_network_connectivity_errors(self, mock_client):
        ...
        assert "Connection failed" in str(exc_info.value) or "ArXiv query failed" in str(exc_info.value)
    ...
    async def test_query_parameter_encoding(self):
        ...
        with patch('capabilities.arxiv_base.httpx.AsyncClient') as mock_client:
            ...
            result = await execute_arxiv_query({"search_query": special_query, "max_results": 5})            # Should complete successfully
            assert result is not None
