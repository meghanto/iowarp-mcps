...
class TestEdgeCases:
    ...
    async def test_download_paper_error_handling(self):
        ...
        with patch('capabilities.download_paper.httpx.AsyncClient') as mock_client:
            ...
            mock_response.raise_for_status.side_effect = ConnectionError("Connection failed")
    ...
    def test_export_utils_edge_cases(self):
        ...
        edge_case_papers = [
            ...
            {"id": "1", "title": "Test", "authors": ["A"], "summary": "S"},  # All fields
            ...
            {"id": "9", "title": "Test\nTitle", "authors": ["Auth\tor"], "summary": "Sum\rmary"},
            {"id": "10", "title": "", "authors": [""], "summary": ""},  # Empty strings in list
        ]
    ...
    async def test_mcp_handlers_json_parsing_errors(self):
        ...
        for invalid_json in invalid_json_inputs:
            ...
            try:
                ...
                await mcp_handlers.download_multiple_pdfs_handler(invalid_json, "/tmp", 5)
            except Exception:
                pass  # JSON parsing error handling
            ...
            for start_date, end_date, category, max_results in edge_cases:
                try:
                    ...
                    if category and mock_query.called:
                        ...
                        if category:
                            assert f"cat:{category} AND submittedDate:" in call_args["search_query"]
